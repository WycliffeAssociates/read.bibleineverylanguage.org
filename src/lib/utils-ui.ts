...
import { type Setter, onCleanup } from "solid-js";
...
export async function getWholeBook({
	...
	try {
		...
		if (originalRepoIndex) {
			...
			const matchingBookIsPopulated = matchingBook?.chapters.every(
				(chap) => !!chap.content,
			);
