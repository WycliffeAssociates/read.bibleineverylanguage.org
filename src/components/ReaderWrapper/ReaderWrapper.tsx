...
export function ReaderWrapper(props: ReaderWrapperProps) {
	...
	const wholeResourceHtml = createMemo(() => {
		const html = readerStore.text
			?.flatMap((book) => book.chapters.map((chap) => chap.content))
	...
	const HTML = createMemo(() => {
		...
		return currentChap?.content || undefined;
	});
	...
	// @ fetching html
	let controller: AbortController; //reuse btw invocations
	...
	function reportScrollPosition(event: Event) {
		...
		previewPane?.dispatchEvent(notifyPreviewPaneOfScrollEvent);
	}
	...
	onMount(async () => {
		if (props.wasPostRequest) {
			...
			let storeQueryParamChapter = storeQueryParamBook?.chapters.find(
				(chap) => chap.label === chapter,
			);
